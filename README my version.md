# elastic-security-lolbin-rundll32-regsvr32-investigation-lab

LOLBin means Living-off-the-Land Binary — a legitimate executable that can potentially be abused to perform actions in ways that may blend into normal system activity.

For this lab we will investigate:
...
What was the parent process?
        ↓
What executable path was used?
        ↓
Did it create child processes?
        ↓
Was there network/file activity?
        ↓
Is the activity actually suspicious?

The important principle remains the same as Lab 01:

The presence of a LOLBin is an investigation signal, not proof of malicious activity.
...
- Examine how `rundll32.exe` and `regsvr32.exe` appear in Elastic endpoint telemetry.
- Investigate LOLBin executions using process-level evidence rather than process names alone.
- Analyze command-line arguments associated with each execution.
- Trace the parent process responsible for launching the LOLBin.
- Compare controlled PowerShell-launched activity with pre-existing Windows system activity.
- Validate executable locations and user contexts associated with LOLBin execution.
- Identify DLL-related command-line activity for further investigation.
- Use ES|QL to perform focused LOLBin hunting across endpoint events.
- Assess whether observed activity represents normal, suspicious, or inconclusive behavior based on available evidence.
- Map the investigated behavior to the relevant MITRE ATT&CK techniques.
- Identify telemetry limitations and avoid conclusions that are not supported by the available evidence.
  
# Lab Scenario

A SOC analyst is investigating Windows endpoint activity involving built-in Windows binaries commonly associated with **Living-off-the-Land Binary (LOLBin)** techniques. The investigation focuses on `rundll32.exe` and `regsvr32.exe`, which have legitimate Windows functions but can also be abused to execute or load malicious content.

The analyst uses **Elastic Security and Elastic Defend** to examine endpoint telemetry and determine how these binaries were executed, which processes launched them, what command-line arguments were used, and under which user context they ran.

Controlled executions of:

- `rundll32.exe /?`
- `regsvr32.exe /?`

are performed to generate safe endpoint telemetry.

The collected telemetry is then compared with other `rundll32.exe` activity already present on the endpoint, including execution originating from `svchost.exe` under the `SYSTEM` account and involving a Windows DLL.
...
- DLL references
- Differences between controlled test activity and existing system activity
- Whether the available evidence supports a suspicious or malicious interpretation

The lab emphasizes that the presence of a LOLBin alone does not establish malicious activity. Execution context and supporting telemetry must be examined before reaching an assessment.
